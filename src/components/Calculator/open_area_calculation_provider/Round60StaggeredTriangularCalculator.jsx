...
export default function Round60StaggeredTriangularCalculator({ title, image }) {
  // Input States[cite: 8]
  ...
  useEffect(() => {
    ...
    if (diaMm > 0 && pitchMm > 0) {
      // Standard Industrial Formula for Round 60° Staggered
      const openArea = (Math.pow(diaMm, 2) * 90.69) / Math.pow(pitchMm, 2);
  ...
  return (
    <CalculatorView title={title} image={image} description="Calculate the open area percentage for Round 60° Staggered Triangular Holes.">
