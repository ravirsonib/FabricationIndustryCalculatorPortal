...
import { Maximize, Activity, Circle, Target, IndianRupee } from 'lucide-react';
...
export default function DutchWovenWeightPerOdLessIdCalculator({ title, image }) {
  ...
  useEffect(() => {
    ...
    const materialConstant = selectedMaterial.materialConstant || 67;
    ...
    const innerDiameterMm = convertToMm(innerDiameter, idUnit);
    
    const wast = parseFloat(wastage) || 0;
    const costPerKg = parseFloat(cost) || 0;

    if (widthMeshMm > 0 && lengthMeshMm > 0 && wireDia1Mm > 0 && wireDia2Mm > 0 && 
        outerDiameterMm > 0 && innerDiameterMm >= 0 && innerDiameterMm < outerDiameterMm) {
      ...
      const rollFactor = ((outerDiameterMm * outerDiameterMm) - (innerDiameterMm * innerDiameterMm)) / 100000.0;

      let weight = meshFactor * rollFactor;
  ...
  }, [materialId, widthMesh, lengthMesh, wireDia1, wireDia2, outerDiameter, innerDiameter, wastage, cost, wd1Unit, wd2Unit, odUnit, idUnit]);

  const handleClear = () => {
    setWidthMesh(''); setLengthMesh(''); setWireDia1(''); setWireDia2('');
    setOuterDiameter(''); setInnerDiameter(''); setWastage(''); setCost('');
  };

  return (
    <CalculatorView title={title} image={image} description="Calculate Weight Per OD (Less ID) for Dutch Woven Wire Mesh.">
