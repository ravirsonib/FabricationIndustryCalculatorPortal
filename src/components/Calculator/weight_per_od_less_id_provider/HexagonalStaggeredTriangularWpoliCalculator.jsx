...
import { Maximize, Activity, Layers, IndianRupee, Circle, Target } from 'lucide-react';
...
export default function HexagonalStaggeredTriangularWpoliCalculator({ title, image }) {
  const [materialId, setMaterialId] = useState('aluminium');
  
  // Inputs
  const [hexHole, setHexHole] = useState('');
  ...
  // Units
  const [hexUnit, setHexUnit] = useState('mm');
  ...
  useEffect(() => {
    ...
    const hh = convertToMm(hexHole, hexUnit);
    const p = convertToMm(pitch, pitchUnit);
    
    let openAreaPercent = 0.0;
    if (hh > 0 && p > 0) {
      const holeArea = hh * hh;
      const pitchArea = p * p;
      if (pitchArea > 0) {
  ...
  }, [materialId, hexHole, pitch, thickness, outerDiameter, perfDiameter, innerDiameter, cost, hexUnit, pitchUnit, thickUnit, odUnit, pdUnit, idUnit]);

  const handleClear = () => { /* Clear all inputs */ };

  return (
    <CalculatorView title={title} image={image} description="Calculate weight per OD (Less ID) for Hexagonal Staggered Triangular holes.">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
        ...
        {/* Hole & Pitch */}
        <CustomInput label="Hexagonal Hole" value={hexHole} onChange={setHexHole} placeholder="e.g. 5" icon={Maximize} unitOptions={COMMON_UNITS} selectedUnit={hexUnit} onUnitChange={setHexUnit} />
        <CustomInput label="Pitch" value={pitch} onChange={setPitch} placeholder="e.g. 10" icon={Activity} unitOptions={COMMON_UNITS} selectedUnit={pitchUnit} onUnitChange={setPitchUnit} />
