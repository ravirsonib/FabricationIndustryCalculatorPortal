...
export default function CapsuleStaggeredTriangularWpoliCalculator({ title, image }) {
  ...
  useEffect(() => {
    ...
    if (wh > 0 && lh > 0 && wp > 0 && lp > 0) {
      ...
      const rectArea = wh * rectWidth;
      const circleArea = Math.PI * (wh / 2) * (wh / 2);
      const holeArea = rectArea + circleArea;

      const pitchArea = wp * lp * 0.5; // Specific to staggered according to Dart
  ...
  }, [materialId, widthHole, lengthHole, widthPitch, lengthPitch, thickness, outerDiameter, perfDiameter, innerDiameter, cost, whUnit, lhUnit, wpUnit, lpUnit, thickUnit, odUnit, pdUnit, idUnit]);

  const handleClear = () => { /* Clear all inputs */ };

  return (
    <CalculatorView title={title} image={image} description="Calculate weight per OD (Less ID) for Capsule Staggered Triangular holes.">
