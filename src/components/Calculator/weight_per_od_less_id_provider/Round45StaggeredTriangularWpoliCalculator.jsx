...
import { Circle, Activity, Layers, IndianRupee, Target } from 'lucide-react';
...
export default function Round45StaggeredTriangularWpoliCalculator({ title, image }) {
  ...
  useEffect(() => {
    ...
    if (hd > 0 && p > 0) {
      ...
      const pitchArea = p * p * 0.70710678118; // p^2 * sin(45)
  ...
  }, [materialId, holeDiameter, pitch, thickness, outerDiameter, perfDiameter, innerDiameter, cost, holeDiameterUnit, pitchUnit, thickUnit, odUnit, pdUnit, idUnit]);

  const handleClear = () => { /* Clear inputs */ };

  return (
    <CalculatorView title={title} image={image} description="Calculate weight per OD (Less ID) for Round 45° Staggered Triangular holes.">
