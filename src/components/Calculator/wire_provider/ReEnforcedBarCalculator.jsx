...
import { Maximize, Circle, IndianRupee } from 'lucide-react';
...
export default function ReEnforcedBarCalculator({ title, image }) {
  ...
  }, [materialId, diameter, length, cost, diameterUnit, lengthUnit]);

  const handleClear = () => {
    setDiameter(''); setLength(''); setCost('');
  };

  return (
    <CalculatorView title={title} image={image} description="Calculate weight for Re-enforced Bar.">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
        ...
        <CustomInput label="Diameter" value={diameter} onChange={setDiameter} placeholder="e.g. 10" icon={Circle} unitOptions={COMMON_UNITS} selectedUnit={diameterUnit} onUnitChange={setDiameterUnit} />
