...
export default function HexagonalWireCalculator({ title, image }) {
  ...
  useEffect(() => {
    ...
    if (tM > 0 && lM > 0) {
      // Area of Hexagon given width across flats (WT)
      const area = 0.86602540378 * (tM * tM);
  ...
  return (
    <CalculatorView title={title} image={image} description="Calculate weight for Hexagonal Wire.">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
        ...
        <CustomInput label="Thickness (Width Across Flats)" value={thickness} onChange={setThickness} placeholder="e.g. 10" icon={Activity} unitOptions={COMMON_UNITS} selectedUnit={thicknessUnit} onUnitChange={setThicknessUnit} />
