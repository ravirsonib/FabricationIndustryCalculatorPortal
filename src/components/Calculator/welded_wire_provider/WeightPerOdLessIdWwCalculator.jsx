...
export default function WeightPerOdLessIdWwCalculator({ title, image }) {
  ...
  useEffect(() => {
    ...
    const id = convertToMm(innerDiameter, idUnit) / 1000;
    ...
    if (pitchWidth > 0 && pitchLength > 0 && od > 0 && od > id) {
      ...
      // Area of OD less ID
      const areaOfMesh = (Math.PI * (Math.pow(od, 2) - Math.pow(id, 2))) / 4;
  ...
  }, [materialId, widthOpening, wireDiameterWidth, lengthOpening, wireDiameterLength, outerDiameter, innerDiameter, wastage, cost, wOpgUnit, wDiaWidthUnit, lOpgUnit, wDiaLengthUnit, odUnit, idUnit]);

  return (
    <CalculatorView title={title} image={image} description="Calculate Weight Per OD (Less ID) for Welded Wire Mesh.">
