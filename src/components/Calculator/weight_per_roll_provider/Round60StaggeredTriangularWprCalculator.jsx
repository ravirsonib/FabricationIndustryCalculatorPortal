...
import { Circle, Activity, Layers, Maximize, IndianRupee } from 'lucide-react';
...
export default function Round60StaggeredTriangularWprCalculator({ title, image }) {
  ...
  useEffect(() => {
    ...
    if (hd > 0 && p > 0) {
      ...
      const pitchArea = p * p * 0.86602540378; // p^2 * sin(60)
      if (pitchArea > 0) {
  ...
  }, [materialId, holeDiameter, pitch, thickness, sheetWidth, sheetLength, perfWidth, perfLength, cost, holeDiameterUnit, pitchUnit, thickUnit, swUnit, slUnit, pwUnit, plUnit]);

  const handleClear = () => { /* Reset fields */ };

  return (
    <CalculatorView title={title} image={image} description="Calculate weight per roll for Round 60° Staggered Triangular holes.">
