...
import { Maximize, Activity, Layers, IndianRupee, Circle } from 'lucide-react';
...
export default function HexagonalStaggeredTriangularWpoCalculator({ title, image }) {
  ...
  useEffect(() => {
    ...
    const h = convertToMm(hexHole, hexUnit);
    const p = convertToMm(pitch, pitchUnit);
    
    let openAreaPercent = 0.0;
    if (h > 0 && p > 0) {
      openAreaPercent = (h * h) / (p * p) * 100.0;
    }
  ...
  }, [materialId, hexHole, pitch, thickness, outerDiameter, perfDiameter, cost, hexUnit, pitchUnit, thickUnit, odUnit, pdUnit]);

  return (
    <CalculatorView title={title} image={image} description="Calculate weight per OD for Hexagonal Staggered holes.">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
        ...
        <CustomInput label="Pitch" value={pitch} onChange={setPitch} placeholder="e.g. 8" icon={Activity} unitOptions={COMMON_UNITS} selectedUnit={pitchUnit} onUnitChange={setPitchUnit} />
      ...
      {/* Results Section (Same as previous component) */}
